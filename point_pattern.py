...
class PointPattern():
    ...
    def average_nearest_neighbor_distance_KDTree(self,mark=None):
        ...
        nearest_distances=[]
        point_list=[(point.x,point.y) for point in points_tmp]
        ...
        for p in point_stack:
            ...
            nearest_distances.append(nearest_neighbor_distance[1])

        nn_distances = np.array(nearest_distances)

        return np.mean(nn_distances)

    def average_nearest_neighbor_distance_np(self,mark=None):
        ...
        for i in range(len(ndarray)):
            ...
            for j in range(len(ndarray)):
                if i==j:
                    ...
                    distance.append(ss.distance.euclidean(ndarray[i], ndarray[j]))
            nearest_distances.append(np.min(distance))
